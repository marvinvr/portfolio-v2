...
<div class="max-w-[85rem] px-4 py-10 md:px-8 lg:px-8 lg:py-14 mx-auto" id="contact-me">
    <div>
      ...
      <div class="mt-12 lg:grid-cols-2 gap-6 lg:gap-16">
        ...
        <div class="divide-y divide-gray-200  w-fit mx-auto">
            {#each iconBlocks as iconBlock}
                
                <!-- Icon Block -->
                <div class="flex gap-x-7 py-6 pr-12 w-full md:w-80">
                    <img src={iconBlock.icon} class="flex-shrink-0 w-6 h-6 mt-1.5 text-gray-800 " alt={iconBlock.title} />
                    <div class="grow">
                    <span class="font-semibold text-gray-800 ">{iconBlock.title}</span>
                    <a class="mt-2 inline-flex items-center gap-x-2 text-sm font-medium text-gray-600 hover:text-gray-800     " href={iconBlock.link}>
